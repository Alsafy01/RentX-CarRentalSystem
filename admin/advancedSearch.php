...
if (!empty($data)) {
    ...
    if (!empty($data['selectedYear'])) {
        $whereConditions[] = "Cars.year = " . (int)$data['selectedYear'];
    }

    if (!is_null($data['selectedStatus'])) {
        $whereConditions[] = "Cars.status = '" . $conn->real_escape_string($data['selectedStatus']) . "'";
    }

    if (!empty($data['pricePerDayFilter'])) {
        $whereConditions[] = "Cars.price_per_day <= " . (float)$data['pricePerDayFilter'];
    }
    ...
    if (!empty($data['selectedBodyShapes'])) {
        $bodyShapeConditions = array_map(function ($bodyShape) {
            return "Cars.body_shape = '" . $bodyShape . "'";
        }, $data['selectedBodyShapes']);
        $whereConditions[] = "(" . implode(" OR ", $bodyShapeConditions) . ")";
    }

    if (!empty($data['selectedColors'])) {
        $colorConditions = array_map(function ($color) {
            return "Cars.color = '" . $color . "'";
        }, $data['selectedColors']);
        $whereConditions[] = "(" . implode(" OR ", $colorConditions) . ")";
    }
	if (!empty($data['reservationDate'])) {
        $whereConditions[] = "Reservations.reservation_date = '" . $conn->real_escape_string($data['reservationDate']) . "'";
    }

    if (!empty($data['customerName'])) {
        $whereConditions[] = "LOWER(Customers.name) LIKE '%" . $conn->real_escape_string(strtolower($data['customerName'])) . "%'";
    }

    if (!empty($data['phone'])) {
        $whereConditions[] = "Customers.phone LIKE '%" . $conn->real_escape_string($data['phone']) . "%'";
    }

    if (!empty($data['customerEmail'])) {
        $whereConditions[] = "LOWER(Customers.email) LIKE '%" . $conn->real_escape_string(strtolower($data['customerEmail'])) . "%'";
    }
